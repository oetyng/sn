...
impl Session {
    ...
    async fn we_have_sufficient_acks_for_cmd(
        ...
        while let Some(msg_resp) = resp_rx.recv().await {
            match msg_resp {
                MsgResponse::CmdAck(ack_src) => {
                    let preexisting =
                        !received_acks.insert(ack_src) || received_errors.contains(&ack_src);
                    debug!(
                ...
                MsgResponse::CmdError(error_src, error) => {
                    let _ = received_errors.insert(error_src);
                    error!(
                        "received error {:?} of cmd {:?} from {:?}, so far {} respones and {} of them are errors",
                        error, msg_id, error_src, received_acks.len() + received_errors.len(), received_errors.len()
                    );
    ...
    async fn check_query_responses(
        ...
        while let Some(msg_resp) = resp_rx.recv().await {
            let (peer_address, response) = match msg_resp {
                ...
                MsgResponse::CmdAck(src) | MsgResponse::CmdError(src, _) => {
                    debug!("Ignoring unexpected response received from {src:?} when awaiting a QueryResponse");
                    continue;
                }
            };

...
pub async fn start_network_and_upload() -> Result<()> {
    ...
    store_data_addresses_to_disk(all_data_put)?;

    // ======================
    // Now the user can add nodes, or muck around with the network as it wishes,
    // as to later read the files from disk again and try get them from the network.
    // ======================
...
fn store_data_addresses_to_disk(all_data_put: Vec<(XorName, [u8; 32])>) -> Result<()> {
    // =============================================
    // Store the data addresses and hashes to disk.
    // =============================================
    use std::{

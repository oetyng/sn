...
const MAX_NODES_CHURNED: u64 = 15;
...
pub async fn run() -> Result<()> {
    ...
    churn_and_verify(all_data_put, sn_launch_tool_args).await?;
...
/// Removes a number of nodes at a time, replacing each of them before removing another one.
/// After each set of churned nodes, the data integrity is verified.
/// 
/// Specifically, we increment the number of nodes replaced, by one for each round, up to `MAX_NODES_CHURNED` at a time.
async fn churn_and_verify(all_data_put: Vec<(XorName, [u8; 32])>, sn_launch_tool_args: Vec<&str>) -> Result<()> {
    // ======================
    // Churn and verifys
    // ======================
    for num_churns in 0..MAX_NODES_CHURNED {
        // ======================
        // Churn nodes
        // ======================

        for _ in 0..num_churns {
            // we remove a node, and replace it
            churn_node(sn_launch_tool_args.clone()).await?;
        }
        ...
        // then we verify the data integrity after the churns
        for (address, hash) in all_data_put.clone() {
...
async fn churn_node(mut sn_launch_tool_args: Vec<&str>) -> Result<()> {
    use rand::{rngs::OsRng, seq::SliceRandom};
        ...
        let mut nodes: Vec<_> = system.processes_by_name("sn_node").collect();
        ...
        let additional_node_count_str = &"1";

        sn_launch_tool_args.push("--add");
        sn_launch_tool_args.push("-n");
        sn_launch_tool_args.push(additional_node_count_str);
        debug!("Adding additional testnet node with args: {:?}", sn_launch_tool_args);
    
        // We can now call the tool with the args
        Launch::from_iter_safe(&sn_launch_tool_args)
            .map_err(|error| eyre!(error))
            .and_then(|launch| launch.run())
            .wrap_err("Error adding node to the testnet")?;
